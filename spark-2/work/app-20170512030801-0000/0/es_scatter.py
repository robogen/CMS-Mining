...
def snapPop(accum, x):
    lastHost = x["LastRemoteHost"]

    if not type(lastHost) == type(np.array([])):
        counter = lastHost[lastHost[lastHost.rfind('.'):].rfind('.'):].rfind('.')

        if not counter == -1:
            lastHost = lastHost[counter + 1:]
    ...
    temp={'meanCpuEff' : np.append(accum["meanCpuEff"], \
          ...
          'endDate' : np.append(accum["endDate"], \
                                x["endDate"]),
          'LastRemoteHost' : np.append(accum["LastRemoteHost"], \
                                       lastHost),
          'meanEventRate' : np.append(accum["meanEventRate"], \
                                            x["meanEventRate"])}
    return temp
            ...
            if not hcc_single.isEmpty():
                for pong in destSite:
                    ...
                    if not hcc_pong.isEmpty():
                        hcc_result = hcc_pong.reduce(snapPop)
                        
                        figH, axH = plt.subplots(2, sharex=True)
                        figH.autofmt_xdate(bottom=0.2, rotation=30, ha='right')
                        axH[0].scatter(hcc_result["beginDate"], \
                                    hcc_result["meanCpuEff"], \
                                    label = hcc_result["LastRemoteHost"])
                        axH[0].set_ylabel("CpuEff")
                        axH[1].set_xlabel(workDate)
                        axH[0].set_title(str(ping + " To " \
                                             + pong))
        
                        axH[1].scatter(hcc_result["beginDate"], 
                                    hcc_result["meanEventRate"])
