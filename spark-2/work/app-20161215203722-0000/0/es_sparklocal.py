...
hcc_rdd = sc.newAPIHadoopRDD(inputFormatClass="org.elasticsearch.hadoop.mr.EsInputFormat",\
                             ...
                             conf=hcc_conf).map(lambda x: x[1]).cache()

atlas_through = sc.newAPIHadoopRDD(inputFormatClass="org.elasticsearch.hadoop.mr.EsInputFormat",\
                                 keyClass="org.apache.hadoop.io.NullWritable",\
                                 valueClass="org.elasticsearch.hadoop.mr.LinkedMapWritable",\
                                 conf=atlas_through_conf).map(lambda x: x[1]) \
                                 .filter(lambda x: "srcSite" in x.keys()) \
                                 .filter(lambda x: "destSite" in x.keys()) \
                                 .cache()
atlas_packet = sc.newAPIHadoopRDD(inputFormatClass="org.elasticsearch.hadoop.mr.EsInputFormat",\
                                keyClass="org.apache.hadoop.io.NullWritable",\
                                valueClass="org.elasticsearch.hadoop.mr.LinkedMapWritable",\
                                conf=atlas_packet_conf).map(lambda x: x[1]) \
                                 .filter(lambda x: "srcSite" in x.keys()) \
                                 .filter(lambda x: "destSite" in x.keys()) \
                                 .cache()
atlas_latency = sc.newAPIHadoopRDD(inputFormatClass="org.elasticsearch.hadoop.mr.EsInputFormat",\
                                 keyClass="org.apache.hadoop.io.NullWritable",\
                                 valueClass="org.elasticsearch.hadoop.mr.LinkedMapWritable",\
                                 conf=atlas_latency_conf).map(lambda x: x[1]) \
                                 .filter(lambda x: "srcSite" in x.keys()) \
                                 .filter(lambda x: "destSite" in x.keys()) \
                                 .cache()
...
print("Started")
with PdfPages('CMS_SparkScatter-%s.pdf' % str(sys.argv[0])) as ps:
    d = ps.infodict()
