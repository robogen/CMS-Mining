...
def main(ps):
    srcSite = hcc_rdd.map(lambda x: (str(x["src"]).lower(), int(0))) \
                     .aggregateByKey(0, lambda k,v: int(v)+k, lambda v,k: k+v) \
                     .collect()
    pp.pprint(srcSite)
    destSite = hcc_rdd.map(lambda x: (str(x["dest"]).lower(), int(0))) \
                      .aggregateByKey(0, lambda k,v: int(v)+k, lambda v,k: k+v) \
                      .map(lambda x: x[0][0]) \
                      .collect()
    prevSite = hcc_rdd.map(lambda x: (str(x["LastRemoteHost"]).lower(), int(0))) \
                      .aggregateByKey(0, lambda k,v: int(v)+k, lambda v,k: k+v) \
                      .map(lambda x: x[0][0]) \
                      .collect()
    for ping in srcSite:
        hcc_single = hcc_rdd.filter(lambda x: str(x["src"]).lower() == ping) \
                            .persist(pyspark.StorageLevel.MEMORY_AND_DISK)
