        ...
        while conTotalRec > 0:
            ...
            for hit in responseCon["hits"]["hits"]:
                if not str(hit["_source"]["Workflow"]) in arrRet:
                    ...
                    lumos = hit["_source"]["meanKEvents"]/hit["_source"]["meanWallClockHr"]
                    arrRet[str(hit["_source"]["Workflow"])] = np.reshape(np.array([lumos,
...
#print(esConAgg("dest"))
def main(utcStart):
    with PdfPages('PDFOut/CMS_VSLumos.pdf') as pc:
        d = pc.infodict()
        ...
        with open("WorkOut/VSLumos.out", "w") as ww:
                    ...
                    for pong in destSites:
                        ...
                        if not type(qResults) == type(None):
                            ...
                            for hit in qResults:
                                ...
                                axsT[1].set_xlabel(str("meanLumosity (" + hit + ")"))
                                ...
                                ww.write(str("X: CpuEff  Y: Lumosity\n"))
                                ...
                                eslope, eintercept, er_value, ep_value, estd_err = stats.linregress(diskUsage[:,0],diskUsage[:,2])
                                ww.write(str("X: EventRate  Y: Lumosity\n"))
